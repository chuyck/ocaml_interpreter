type stackValue = BOOL of bool | INT of int | ERROR | S of string | N of string | UNIT | CLOSURE of (stackValue * (command list) * ((string*stackValue)list) list)
and command = ADD | SUB | MUL | DIV | PUSH of stackValue | POP | REM | NEG | SWAP | TOSTRING | PRINTLN | QUIT | CAT | AND | OR | NOT | EQUAL | LESSTHAN | BIND | IF | LET | END | FUN of (string * stackValue ) | CALL | RETURN | FUNEND
(* add more commands later on *)

(* closure has: ( input paramerter * function content * current memory ) *)
...
let rec string2command (str:string) : command =
  match str with 
  | "add" -> ADD
  | "sub" -> SUB
  | "mul" -> MUL
  ...
  | "return" -> RETURN
  | "funEnd" -> FUNEND
  | _ -> let sub_str = String.sub str 0 3 in
    match sub_str with 
    | "pus" -> PUSH (string2stackVal (String.sub str 5 ((String.length str) - 5)))
    | "fun" -> let sList = String.split_on_char ' ' str in (* split on space *)
      ( match sList with 
        | a::b::c::more -> 
          (match string2stackVal c with
          | ERROR -> PUSH ERROR
          | _ -> FUN (b, string2stackVal c) )
        | _ -> PUSH ERROR )
    | _ -> PUSH ERROR
...
let nameAsClosure name mem : stackValue = 
  let sv = nameFind name mem in
  match sv with
  | CLOSURE(v) -> CLOSURE(v)
  | _ -> ERROR

let rec findFunEnd comms : command list =
  match comms with 
  | FUNEND::moreComms -> moreComms
  | someComm::moreComms -> findFunEnd moreComms
  | [] -> []

let rec getFun comms funList : command list =
  match comms with
  | FUNEND::moreComms -> funList
  | someComm::moreComms -> getFun moreComms (someComm::funList)
  | [] -> []

let makeClosure comms arg mem : stackValue =
  match arg with
  | N a ->
    (match nameFind a mem with 
    | ERROR -> ERROR
    | b -> let func = (getFun comms []) in CLOSURE ((arg), (func), mem) )
  | _ -> let func = (getFun comms []) in CLOSURE ((arg), (func), mem)
  (* match arg with find Fun and add to function closure*)

let rec putOnStack (s1) (s2) =
  match s1 with
  | [] -> s2
  | a::more -> putOnStack more (a::s2)
...
let interpreter ((input:string),(output:string)) : unit = 
  ...
  let rec processor (commandList: command list) (stack: stackValue list list)  (mem: ((string*stackValue) list ) list) : unit =
    ...
    | (PUSH CLOSURE a::moreComms, s::more, m) -> processor moreComms ( (CLOSURE a::s)::more ) m (* may be a problem *)
    ...
    | ((FUN (str, param))::moreComms, (N(funName)::input::s)::more, m ) -> let close = (makeClosure moreComms input m) in processor (findFunEnd moreComms) (s::more) (((funName , close)::[])::m)
    | ((FUN (str, param))::moreComms, s::more, m) -> processor moreComms ((ERROR::s)::more) m
    | (FUNEND::moreComms, s::more, m) -> processor moreComms ((UNIT::s)::more) m
    | (RETURN::moreComms, s1::s2::more, m::moreMem) -> let a = putOnStack s1 s2 in processor moreComms (a::more) moreMem
    | (RETURN::moreComms, s::more, m) -> processor moreComms ((ERROR::s)::more) m
    | (CALL::moreComms, (arg::N(funName)::s)::more, m) -> 
      (match nameAsClosure funName m, arg with
        | ERROR, _ -> processor moreComms ((ERROR::arg::N(funName)::s)::more) m
        | _,ERROR -> processor moreComms ((ERROR::arg::N(funName)::s)::more) m
        | clos, N(ar) -> (match clos with 
        | CLOSURE (input , (contents) , ((memory))) -> let comb = putOnStack contents moreComms in processor (PUSH (nameFind ar m):: PUSH clos::BIND ::comb) ([]::stack) ([]::memory)
        | _ -> processor moreComms ((ERROR::arg::N(funName)::s)::more) m)  
        | clos,ar -> (match clos with 
              | CLOSURE (input , (contents) , ((memory))) -> let comb = putOnStack contents moreComms in processor (PUSH ar:: PUSH clos::BIND ::comb) ([]::stack) ([]::memory)
              | _ -> processor moreComms ((ERROR::arg::N(funName)::s)::more) m ) )
        
    | (CALL::moreComms, (s)::more, m) -> processor moreComms ((ERROR::s)::more) m
